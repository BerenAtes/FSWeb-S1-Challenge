...
function indekseGoreFenomen(fenomenler, sayı) {
  return `${sayı}. indekste bulunan fenomen: ${fenomenler[sayı].profile}`;
}
...
function profilListesi(fenomen) {
  ...
  for (let i = 0; i < fenomen.length; i++) {
    newFenomen.push(fenomen[i].profile);
  }
  return newFenomen;
}
...
function enFenomenler(fenomenler) {
  ...
  for (let i = 0; i < fenomenler.length; i++)
    if (fenomenler[i].followers > 100000000) {
      enfeno.push(fenomenler[i].profile);
    }
  return enfeno;
}
